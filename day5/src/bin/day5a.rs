...
fn main() {
    ...
    for instruction in parts.next().unwrap().lines() {
        ...
        stacks.execute_instruction_9000(instruction).unwrap();
    }
...
move 1 from 1 to 2";
        ...
        assert_eq!(*stacks.stacks.get(0).unwrap(), LinkedList::from(['C']));
        assert_eq!(*stacks.stacks.get(1).unwrap(), LinkedList::from(['M']));
        assert_eq!(*stacks.stacks.get(2).unwrap(), LinkedList::from(['P', 'D', 'N', 'Z']));

        let top_crates = stacks.get_top_crates();
        assert_eq!(top_crates, vec![Some('C'), Some('M'), Some('Z')]);
